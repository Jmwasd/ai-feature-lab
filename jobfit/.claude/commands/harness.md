jobfit은 Harness 프레임워크를 사용한다. 아래 워크플로우에 따라 작업을 진행하라.

경로는 전부 `jobfit/` 기준이다. 이 저장소는 프로젝트 여러 개가 한 폴더에 있으므로 **`jobfit/` 밖의 파일을 읽거나 고치지 마라.**
...
1. 위 AC 커맨드를 실행한다.
2. 아키텍처 체크리스트를 확인한다:
   - ARCHITECTURE.md 디렉토리 구조와 레이어 방향(`app → services → 외부`, `lib`은 잎)을 따르는가?
   - ADR 기술 스택과 결정을 벗어나지 않았는가?
   - CLAUDE.md CRITICAL 규칙을 위반하지 않았는가? 특히:
     - 근거 텍스트를 LLM이 쓰게 하지 않았는가 (블록 ID만 받고 텍스트는 코드가 가져오는가)
...
- `feat/jobfit/{task-name}` 브랜치 생성/checkout (저장소 `/branch` 규칙: phase 브랜치는 디렉토리명을 그대로 쓴다)
...
- 2단계 커밋 — 코드 변경(`feat(jobfit):`)과 메타데이터(`chore(jobfit):`)를 분리 커밋. **scope는 항상 `jobfit`이고 phase명이 들어가지 않는다**
- `git add` 범위를 `jobfit/` 하위로 한정 — 모노레포라서 전체 add는 남의 프로젝트 변경을 끌고 들어온다

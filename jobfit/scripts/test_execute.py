...
class TestCheckoutBranch:
    ...
    def test_already_on_branch(self, executor):
        self._mock_git(executor, [
            MagicMock(returncode=0, stdout="feat/jobfit/0-mvp\n", stderr=""),
        ])
...
class TestCommitStep:
    def test_two_phase_commit(self, executor):
        ...
        assert "feat(jobfit):" in commit_calls[0][2]
        assert "chore(jobfit):" in commit_calls[1][2]
...
# ---------------------------------------------------------------------------
# 저장소 규칙 (브랜치명 · 커밋 scope · git add 범위)
# ---------------------------------------------------------------------------

class TestRepoConventions:
    def test_branch_name_is_type_project_phasedir(self, executor):
        assert executor._branch_name() == "feat/jobfit/0-mvp"

    def test_branch_uses_phase_dir_not_phase_name(self, executor):
        """phase 이름은 'mvp'지만 브랜치는 디렉토리명 '0-mvp'를 쓴다."""
        assert executor._phase_name == "mvp"
        assert "0-mvp" in executor._branch_name()

    def test_commit_scope_is_project_not_phase(self, executor):
        calls = []
        def fake_git(*args):
            calls.append(args)
            if args[:2] == ("diff", "--cached"):
        ...
        assert all("(jobfit)" in m for m in msgs)
        assert not any("(mvp)" in m for m in msgs)

    def test_git_add_is_scoped_to_project_dir(self, executor):
        """모노레포라 add -A 전체는 다른 프로젝트 변경을 끌고 들어온다."""
        calls = []
        def fake_git(*args):
            calls.append(args)
            if args[:2] == ("diff", "--cached"):

...
`scripts/execute.py`가 `phases/{task-name}/` 안의 step을 순차 실행한다. 각 step은 `codex exec`로 시작되는 독립 세션이며, 이 파일(`AGENTS.md`)과 `docs/*.md`가 매 step 프롬프트에 가드레일로 주입된다.

step 세션은 다음을 지켜야 한다.

- 해당 step에 명시된 작업만 수행한다. 추가 기능·파일을 만들지 않는다.
- AC(Acceptance Criteria)에 적힌 커맨드를 직접 실행해 검증한다.
- `phases/{task-name}/index.json`의 해당 step status를 업데이트한다.
  - AC 통과 → `"completed"` + `"summary"`(산출물 한 줄 요약)
  - 3회 수정 시도 후에도 실패 → `"error"` + `"error_message"`
  - 사용자 개입 필요(API 키, 인증, 수동 설정) → `"blocked"` + `"blocked_reason"` 기록 후 즉시 중단
- 타임스탬프(`created_at`, `started_at`, `completed_at`, `failed_at`, `blocked_at`)는 `execute.py`가 기록한다. 직접 쓰지 않는다.

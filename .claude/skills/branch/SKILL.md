...
제목 한 줄로는 부족하다. **여기 쓰는 본문이 그대로 squash 커밋의 본문이 된다** — GitHub 메모가 아니라 `git log`에 남는 기록이다. 제목 초안과 함께 본문 초안도 낸다.

두 절뿐이고 **본문 전체가 3~5줄이다.** 오타 수정이면 "왜" 한 줄로 끝내고 "무엇을"은 지운다.
...
<이 변경이 필요한 이유. 버린 선택지가 있으면 그것까지.>
```

**Why:** `git log`에서 한눈에 들어와야 한다. 검증 로그·후속 작업처럼 리뷰 때만 필요한 것은 PR 코멘트에 남긴다.

- 한글로 쓴다. 제목에 쓴 어휘를 본문에서도 그대로 쓴다.
- CRITICAL: **"왜"를 생략하지 마라.** "무엇을"은 diff를 보면 알 수 있지만 "왜"는 코드 어디에도 없다. 되짚는 시점은 대개 한참 뒤고, 그때 읽는 사람은 작성자 자신이다.
- **검증했다고 지어내지 마라.** 코멘트에 적을 때도 실제로 돌린 것만 쓴다. 안 돌려봤으면 "검증 안 함"이라고 쓴다.
...
- **squash merge만 쓴다.** GitHub Settings에서 Squash merging만 남기고 merge commit·rebase 버튼은 꺼둔다. 함께 `squash_merge_commit_title=PR_TITLE` · `squash_merge_commit_message=PR_BODY`로 둔다.
  ...
  **Why:** PR 하나가 커밋 하나가 되고, 그 커밋이 PR 본문을 통째로 싣는다. main 히스토리가 곧 결정 기록이 된다. 중간 커밋은 접히지만 GitHub PR 페이지에 그대로 남는다 — 접는 대신 "왜"를 얻는 거래다. step 커밋이 수십 개인 harness에서 이득이 가장 크다.

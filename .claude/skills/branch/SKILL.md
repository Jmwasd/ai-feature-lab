...
<2~3줄. 이 변경이 필요한 이유. 버린 선택지가 있으면 그것까지.>
```

**Why:** `git log`에서 한눈에 들어와야 한다. 길면 읽히지 않고, 안 읽히는 기록은 없는 것과 같다. 상세한 설명·검증 로그·후속 작업 목록은 PR 코멘트에 남긴다 — 리뷰 때 필요한 것과 1년 뒤 로그에서 필요한 것은 다르다.
...
- **검증했다고 지어내지 마라.** 본문에 검증을 적을 자리는 없지만, PR 코멘트에 남길 때도 실제로 돌린 것만 적는다. 안 돌려봤으면 "검증 안 함"이라고 쓴다.
...
- 3~5줄이 상한이지 목표가 아니다. 오타 수정 PR이면 "왜" 한 줄로 끝내고 "무엇을"은 지운다. 줄 수를 채우려고 늘려 쓰지 마라.
